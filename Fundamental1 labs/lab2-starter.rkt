...
(define (string-starts-with? x y)
  (cond [(string=? x y) true]
        [(< (string-length x)(string-length y)) false]
        [(string=? (substring x 0 (string-length y)) y) true]
        [(string=? y "") true]
        [else false]
        ))
...
(define (draw-door y)
  (cond [(string=? DS-CLOSED y) DOOR-CLOSED]
        [(string=? DS-LOCKED y) DOOR-LOCKED]
        [(string=? DS-OPEN y) DOOR-OPEN]))
