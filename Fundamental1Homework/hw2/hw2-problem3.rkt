...
;;simple-wordle-done?:SW -> Boolean
;;accepts a SimpleWordle and returns #true only if it is SW-CORRECT or SW-INCORRECT
...
(define (simple-wordle-done? sw)
  (cond [(string=? sw SW-CORRECT) #true]
...
;;draw-simple-wordle: string->image
;;produce a appropriate face to represent letter status.
...
(define (draw-simple-wordle sw)
  (cond [(string=? sw SW-A)
         (overlay (above (text (string-upcase sw) 80 "black")
                         (text (simple-wordle/eval sw) 50 "black")) background)]
        [(string=? sw SW-B)
         (overlay (above (text (string-upcase sw) 80 "black")
                         (text (simple-wordle/eval sw) 50 "black")) background)]
        [(string=? sw SW-C)
...
(define (key-simple-wordle/letter sw ke)
  (cond [(string=? ke "\b") SW-EMPTY]
        [(string=? ke "\r") (simple-wordle/eval sw)]
        [else sw]))
