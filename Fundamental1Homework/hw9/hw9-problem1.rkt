...
(define (read-dictionary d)
  (if (file-exists? d)
      (map (lambda (x) (string-upcase x)) (read-lines d))
      '()))
...
;  (ormap (lambda (l) (string=? s l)) los))|#
#|
(define (score answer word)
  (local [(define (lopair character)
            (make-pair character
                       (if (string-in-list? character (explode word))
                           (condition character)
                           LS-WRONG)))
          (define (condition character word)
            (if (string=? (first (explode word)) character)
                LS-RIGHT
                LS-MISPLACED))]
    (map lopair (explode answer))))|#

;;condition: String String String -> List
;;set up a make-pair list

(define (condition character1 character2 word)
  (cond
    [(string=? character1 character2) (make-pair character1 "right")]
    [(string-in-list? character1 (explode word)) (make-pair character1 "misplaced")]
    [else (make-pair character1 "wrong")]))

(define (score answer word)
  (mymap2 (explode answer) (explode word) (lambda (x y) (condition x y word))))
...
(define (partial-guess->full-guess string number)
  (implode (append (explode string)
                   (build-list (- number (string-length string)) (lambda (x) " ")))))
...
(define (key-wordle WS ke words number)
  (local
    [;;hfunction1: Wordlestatus -> Wordlestatus
     ;;Determine the status of wordle
     (define (hfunction1 WS)
       (if (string=? "" (wordle-current WS))
           WS 
           (make-wordle (wordle-past WS)
                        (substring (wordle-current WS) 0 (- (string-length (wordle-current WS)) 1)))))
     
     ;;hfunction2: Wordlestatus -> Wordlestatus
     ;;Determine the state of the wordle after entering the word
     (define (hfunction2 WS)
       (if (string-in-list? (wordle-current WS) words)
           (make-wordle (append (wordle-past WS) (list (wordle-current WS))) "") WS))
     
     ;;26characters: 26characters -> Boolean
     ;;Determine whether the input belongs to the 26 letters on the keyboard
     (define (26characters ke)
       (string-in-list? (string-downcase ke)
                        (explode "abcdefghijklmnopqrstuvwxyz"))) 

     ;;hfunction3: Wordlestatus -> Wordlestatus
     ;;put input word into current wordle-status
     (define (hfunction3 WS)
       (if
        (< (string-length (wordle-current WS)) number)
        (make-wordle (wordle-past WS) (string-append (wordle-current WS) (string-upcase ke))) WS))]
    (cond
      [(string=? "\b" ke) (hfunction1 WS)]
      [(string=? "\r" ke) (hfunction2 WS)]
      [(26characters ke) (hfunction3 WS)]
      [else WS])))
...
(define (end-wordle? WS string number)
  (cond
    [(>= (length (wordle-past WS)) number) #true]
    [(string-in-list? string (wordle-past WS)) #true]
    [else #false]))
...
(check-expect (draw-end-wordle (make-wordle (list "hello" "world") "") "hello" 5)
              (overlay (text (string-append "WON 2correct") 60 "green") (square 800 "solid" "black")))
(define (draw-end-wordle WS word number)
  (local  [(define won (overlay (text (string-append "WON "
                                                     (number->string (length (wordle-past WS)))
                                                     "correct") 60 "green")
                                (square 800 "solid" "black")))
           (define lost (overlay (text "LOST" 60 "red")
                                 (square 800 "solid" "black")))]
    (cond [(string-in-list? word (wordle-past WS)) won]
          [else lost]))) 
...
(define (draw-wordle WS string number)
  (local [;;Iowords: answer word -> Image
          ;;Generate images of letters
          (define (Iowords answer word)
            (row->image (map (λ (l) (make-pair l lsp->image)) (score answer word))))
          ;;guess1: wordle-status word number -> Image
          ;;Generate a list of image of letters
          (define (guess1 WS word number)
            (map (λ (l) (Iowords l word)) (wordle-past WS)))
          ;;guess2: worlde-status word number -> Image
          ;;Generate blank boxes images
          (define (guess2 WS word number)
            (partial-guess->image (wordle-current WS) (string-length word)))
          ;;guess3: wordle-status word number -> Image
          ;;Generate the image that combine guess1 and guess2
          (define (guess3 WS word number)
            (stack/v (build-list (- (sub1 number) (length (wordle-past WS)))
                                 (λ (l) (partial-guess->image "" (string-length word))))))]
    (overlay (stack/v (reverse (cons (guess3 WS string number)
                                     (cons (guess2 WS string number) (guess1 WS string number)))))
             (square 800 "solid" "black")))) 
